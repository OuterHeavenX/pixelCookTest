...
function drawBattleUi() {
  ...
  const ly = panelY - 22, lx = 6 + touchGutter();
  drawWindow(lx, ly, textWidth(lamp) + 22, 18, { tone: 'dark' });
  if (Battle.lantern) { ctx.fillStyle = 'rgba(255,215,90,0.35)'; ctx.fillRect(lx + 5, ly + 3, 10, 11); }
  ctx.fillStyle = Battle.lantern ? '#ffd75a' : '#3a3a5a';
  ctx.fillRect(lx + 7, ly + 5, 6, 7);
  drawText(lamp, lx + 18, ly + 5, Battle.lantern ? '#f6e2a8' : '#8c8cb8');
